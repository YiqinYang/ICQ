...
class SquashedGaussianMLPActor(nn.Module):
    ...
    def get_logprob(self, obs, actions):
        ...
        logp_pi -= (2 * (np.log(2) - actions - F.softplus(-2 * actions))).sum(
            axis=1)

        return logp_pi


class icqMLPActor(nn.Module):
    def __init__(self, obs_dim, act_dim, hidden_sizes, activation, act_limit):
        super().__init__()
        self.net = mlp([obs_dim] + list(hidden_sizes), activation, activation)
        self.mu_layer = nn.Linear(hidden_sizes[-1], act_dim)
    ...
    def forward(self, obs, deterministic=False, with_logprob=True):
        ...
        log_std = self.min_log_std + log_std * (self.max_log_std -
                                                self.min_log_std)
        std = torch.exp(log_std)
...
class MLPActorCritic(nn.Module):
    def __init__(self,
        ...
        if special_policy is 'icq':
            self.pi = icqMLPActor(obs_dim, act_dim, (256, 256, 256, 256),
        ...
        self.q2 = MLPQFunction(obs_dim, act_dim, hidden_sizes,
                               activation).to(device)
        self.v = MLPVFunction(obs_dim, act_dim, hidden_sizes,
                              activation).to(device)
